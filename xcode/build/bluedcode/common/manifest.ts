...
export type TransformLedger = {
  version: 1
  modules: Array<{ target: string; file: string; transforms: number }>
  records: Array<{ target: string; id: string; file: string; hits: number; [key: string]: unknown }>
}
...
export type ReleaseManifest = {
  ...
  transformation: { ledger: TransformLedger; sha256: string }
  audit: {
    output: AuditReport
...
function validateReleaseManifest(manifest: ReleaseManifest) {
  if (
    ...
    "artifact,audit,baseline,build,product,release,schemaVersion,transformation"
  ) {
