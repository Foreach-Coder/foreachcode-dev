...
export async function resolveBuildIdentity(
  ...
  if (request.channel === "dev") {
    ...
    return {
      ...
      commit,
      shortCommit,
      artifactName: `BluedCode-Dev-${buildVersion}-windows-x64-portable.exe`,
    }
  }
  ...
  return {
    ...
    commit,
    shortCommit,
    artifactName: `BluedCode-${buildVersion}-windows-x64-portable.exe`,
    tag: `bluedcode-v${version}-${request.release}`,
  }
}
...
export async function assertTrackedBaseline(git: Git): Promise<void> {
  ...
  const overlays = await requireGit(
    git.run([
      "ls-files",
      "--others",
      "--ignored",
      ...
      "--",
      "packages/brand",
      "packages/opencode/script/build-config.ts",
    ]),
    "无法检查 ignored overlay",
  )
  if (overlays.trim()) throw new Error("检测到不允许的 ignored overlay")
}
