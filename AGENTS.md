...
- 根仓维护的 BluedCode 品牌源资源放在 `xcode/build/bluedcode/`；这里只允许保存图标、Wordmark、品牌视觉配置和对应资源摘要，不保存 TypeScript、JavaScript、Shell、PowerShell 等可执行构建代码。
- `opencode/xcode/build/bluedcode/` 是构建框架、版本适配器、测试和子仓品牌资源的唯一代码实现位置。子仓不能依赖父级根仓路径，必须支持独立 clone 后构建。
- 根仓品牌源资源同步到子仓时必须逐文件校验，且不得在未确认的情况下覆盖子仓资源；构建代码不在根仓与子仓之间做双向快照或复制。
- 子仓中的通用构建框架与版本适配器必须分离。通用能力放在公共框架，目标 tag 的路径、语义指纹、替换规则和结构性钩子放在对应版本目录。
